...
from typing import FrozenSet, List, Set, Union, Optional, NamedTuple
...
class AvroMessageSchemas2:
    ...
    # this is used just for testing purposes, showing that the avro name registry fills out during reading
    #  of all the files
    @classmethod
    def _get_avro_schema_namespaces_by_avro_name_element(cls) -> FrozenSet[str]:
        # rememeber - the namespace property will not vary for elements within a given place - may only be one
        return frozenset([x.avro_schema.namespace for x in cls._avro_schema_list])

    @classmethod
    def get_avro_schema_namespaces(cls) -> FrozenSet[str]:
        return frozenset([k for k, v in cls._known_avro_schema_dot_names.names.items()])

    @classmethod
    def get_schema_by_full_namespace(cls,
                                     namespace: str) -> avro.schema.Schema:
        for this_namespace, this_schema in cls._known_avro_schema_dot_names.names.items():
            if this_namespace == namespace:
                return this_schema
    ...
    @classmethod
    def get_matching_schema_record_by_family_and_name(cls,
                                                      schema_family: AvroMessageSchemaFamily,
                                                      schema_name: str) -> AvroMessageSchemaRecord:
        # iterate through and find our record - it is a kind of compound key (we won't have multiple names
        # in the same family
        for this_schema_record in cls._avro_schema_list:
            if this_schema_record.avro_schema_family_name == schema_family and \
                    this_schema_record.avro_schema_name == schema_name:
                return this_schema_record
        ...
        raise KeyError('Unable to match schema entry against the matching criteria' +
                       ...
                       (os.linesep + '\t').join(
                           [x.avro_schema_family_name.name + ': ' + x.avro_schema_name
                            for x in cls._avro_schema_list]))
    ...
    def __init__(self,
        ...
        raise RuntimeError('Class ' + type(self).__name__ + ' should not be instantiated - use as class')
    ...
    def __new__(cls,
        ...
        if type(schema_configuration_record.schema_extension) is not str:
            raise RuntimeError('Caller must provide a string specifying extension to be matched for avro schema files' +
                               os.linesep + 'Specify empty string if no extenesion' +
                               os.linesep + 'Type of data provided: ' +
                               type(schema_configuration_record.schema_extension).__name__)

        # allow a completely empty string to be used signifying no extension
        cls._schema_extension = (os.extsep + schema_configuration_record.schema_extension) \
            if (not schema_configuration_record.schema_extension.startswith(os.extsep) and
                len(schema_configuration_record.schema_extension) > 0) \
            else schema_configuration_record.schema_extension

        if not isinstance(schema_configuration_record.schema_subfolders, list) and \
                not isinstance(schema_configuration_record.schema_subfolders, set) and \
                schema_configuration_record.schema_subfolders is not None:
            raise TypeError(
                'Caller must provide schema_subfolders parameter as None or initialize with a list or set of strings'
            )

        # if caller specifies None as the schema_subfolders parameter, we will search through every directory
        if schema_configuration_record.schema_subfolders is None or \
                len(schema_configuration_record.schema_subfolders) == 0:
            cls._schema_subfolders = sorted(AvroMessageSchemaFamily.enumerate_schema_family_names())
            print('Scanning all subfolders: ' + os.linesep + ','.join([x for x in cls._schema_subfolders]))

        # initialize the names registry - it will be updated
        cls._known_avro_schema_dot_names: avro.schema.Names = avro.schema.Names()

        # create the dictionary we will use to key schemas by their name - it is two level dictionary keyed
        #  first by the schema folder as text and then by the schema Name property from in the properly validated file
        avro_schema_list: List[AvroMessageSchemaRecord] = []

        for schema_type_counter, this_schema_subfolder in enumerate(cls._schema_subfolders, start=1):
            print('Starting scan of AVRO schema subfolder "' + this_schema_subfolder + '"')
            schema_base_import_path = avro_schemas
            try:
                # pep erroneously complains  about this syntax if you use avro_schemas without being in string
                with path('emerald_message.avro_schemas', this_schema_subfolder) as schema_subfolder:
                    if schema_configuration_record.debug:
                        print('The AVRO schema folder path = ' + str(schema_subfolder) + os.linesep +
                              'Type = ' + type(schema_subfolder).__name__)
                    ...
                    subdirectory_list = []
                    if schema_configuration_record.debug:
                        print('Will scan ' + str(schema_subfolder))
                    for entry in schema_subfolder.iterdir():
                        if not entry.name.startswith('.') and entry.is_dir():
                            subdirectory_list.append(entry.name)
                    ...
                    # sort the list alphabetically
                    subdirectory_list = sorted(subdirectory_list, reverse=False)
                    print('For schema subfolder "' + schema_subfolder.name +
                          '", the list of subfolders: ' + str(subdirectory_list))
                    ...
                    if schema_configuration_record.debug:
                        print('The schema paths to search: ' + str(schema_search_path_list))

                    for this_search_path_counter, this_search_path in enumerate(schema_search_path_list, start=1):
                        if schema_configuration_record.debug:
                            print('Scanning search path #' + str(this_search_path_counter) + ': ' +
                                  str(this_search_path))
                        ...
                        with os.scandir(this_search_path) as avro_search_path_iterator:
                            for schema_file_counter, schema_file in enumerate(avro_search_path_iterator, start=1):
                                if schema_file.is_dir():
                                    if schema_configuration_record.debug:
                                        print('Skipping entry "' + os.path.basename(schema_file.name) +
                                              '" for search path ' +
                                              os.path.basename(this_search_path) +
                                              ': is a directory and nesting not supported')
                                    continue
                                if not os.path.splitext(schema_file)[1] == cls._schema_extension:
                                    if schema_configuration_record.debug:
                                        print('Skipping entry "' + os.path.basename(schema_file.name) +
                                              '" for search path ' +
                                              os.path.basename(this_search_path) + ': extension "' +
                                              os.path.splitext(schema_file.path)[1] +
                                              '" does not match schema extension  "' +
                                              cls._schema_extension + '"')
                                    continue

                                print('Scanning file ' + schema_file.name + ' in path ' + this_search_path)
                                ...
                                try:
                                    ...
                                    avro_schema = \
                                        avro.schema.SchemaFromJSONData(json_data=schema_file_json,
                                                                       names=cls._known_avro_schema_dot_names)
                                except json.JSONDecodeError as jdex:
                                    raise EmeraldSchemaParsingException(
                                        'Schema file "' + os.path.basename(
                                            schema_file) + '" cannot be read as valid JSON' +
                                ...
                                if schema_configuration_record.debug:
                                    print('Schema file #' + str(schema_file_counter) + ' contents: ' +
                                          os.linesep + str(avro_schema) + os.linesep +
                                          'Type = ' + type(avro_schema).__name__)
            ...
            print('Total schema count deserialized: ' + str(len(avro_schema_list)))

            #
            #  Design note - we can't really make this immutable as a collection / sequence because
            #  the underlying RecordSchema types in the AvroMessageSchemaRecords are not hashablee
            #  However, we will block access programmatically to the element avro_schema_list and use it
            #  in frozen dataclasses only
            #
            cls._avro_schema_list: List[AvroMessageSchemaRecord] = avro_schema_list
            print('Known schema names (based on avro.schema.Names registry: ' + os.linesep +
                  str(cls._known_avro_schema_dot_names.names))
